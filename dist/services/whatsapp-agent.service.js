...
export class WhatsAppAgentService {
    agentService;
    conversationMap = new Map(); // phone -> conversationId
     ...
     */
    async handleIncomingMessage(message, storeId) {
        try {
            const db = getDb();
            ...
            const response = await this.agentService.executeAgentic(conversationId, customer.id, storeId, message.text);
            if (!response.success) {
