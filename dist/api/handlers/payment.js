...
import { z } from "zod";
import { ValidationError, NotFoundError, PaymentError, ErrorCode, } from "../../lib/errors";
// Validation schemas
 ...
 * Get payment details
 */
export async function getPayment(c) {
    try {
        const user = c.get("user");
        if (!user) {
            return c.json({ success: false, error: "Unauthorized" }, 401);
 ...
 */
export async function checkPaymentStatus(c) {
    try {
        const user = c.get("user");
        if (!user) {
            return c.json({ success: false, error: "Unauthorized" }, 401);
 ...
 */
export async function midtransWebhook(c) {
    try {
        const body = await c.req.json();
