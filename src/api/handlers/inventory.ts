 ...
 */
export async function getInventoryHandler(c: Context) {
  try {
    ...
    if (!storeId || !variantId) {
      throw new ValidationError(
        "Missing required parameters: storeId, variantId",
      );
    }
    ...
    const reservations = await inventoryService.getOrderReservations(
 ...
 * }
 */
export async function reserveStockHandler(c: Context) {
  try {
    const storeId = c.req.param("storeId");
...
export async function adjustInventoryHandler(c: Context) {
  try {
    ...
    const quantityChange = validated.quantity_change as number;
 ...
 */
export async function getMovementsHandler(c: Context) {
  try {
    const storeId = c.req.param("storeId");
    ...
    if (isNaN(storeIdNum) || isNaN(variantIdNum) || isNaN(limit)) {
      throw new ValidationError(
        "storeId, variant_id, and limit must be valid numbers",
      );
    }
 ...
 */
export async function releaseReservationHandler(c: Context) {
  try {
    const storeId = c.req.param("storeId");
    ...
    if (!storeId || !reservationId) {
      throw new ValidationError(
        "Missing required parameters: storeId, reservationId",
      );
    }
    ...
    if (isNaN(storeIdNum) || isNaN(reservationIdNum)) {
      throw new ValidationError(
        "storeId and reservationId must be valid numbers",
      );
    }
 ...
 */
export async function checkLowStockHandler(c: Context) {
  try {
    const storeId = c.req.param("storeId");
