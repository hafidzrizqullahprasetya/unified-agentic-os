 ...
 */
export async function whatsappWebhookVerify(c: Context) {
  try {
 ...
 */
export async function whatsappWebhookReceive(c: Context) {
  try {
...
async function handleMessageAsync(
  storeId: number,
  customerId: number,
  message: Awaited<ReturnType<typeof whatsAppService.parseIncomingMessage>>,
) {
  if (!message) return;
  ...
  try {
    ...
    const { whatsAppAgentService } =
      await import("@/services/whatsapp-agent.service");

    const agentResponse = await whatsAppAgentService.handleIncomingMessage(
      message,
      storeId,
    );
    ...
    try {
      await whatsAppService.sendMessage(
        customer[0].phone,
        "Terjadi kesalahan dalam memproses pesan Anda. Silakan coba lagi nanti atau hubungi admin.",
      );
    } catch (innerError) {
...
async function handleOrderRequest(
  ...
  try {
    ...
    const order = await whatsAppService.createOrderFromMessage(
      storeId,
      customerId,
      details.productId,
      details.quantity,
    ...
    const orderWithProducts = await whatsAppService.getOrderWithProducts(
      storeId,
      order.id,
    );
    ...
    if (orderWithProducts) {
      ...
      const paymentResponse = await paymentService.createPayment({
        ...
        method: "qris",
        customerEmail:
          customer[0].email || `${customer[0].phone}@whatsapp.local`,
      ...
      const confirmMessage = whatsAppService.formatOrderConfirmation(
        order,
        orderWithProducts.items,
        paymentResponse.redirectUrl || paymentResponse.snapToken,
      );
    ...
    try {
      await whatsAppService.sendMessage(
        customer[0].phone,
        "Maaf, terjadi kesalahan dalam memproses pesanan. Silakan coba lagi nanti.",
      );
    } catch {
      // Ignore send errors
    }
...
async function handleStatusRequest(
  ...
  try {
    ...
    if (!orderNumber) {
      await whatsAppService.sendMessage(
        customer[0].phone,
        "Mohon sertakan nomor pesanan untuk tracking.\nContoh: `status #123` atau `track 123`",
      );
      return;
    }
