...
export class InventoryService {
   ...
   */
  async getStockLevel(variantId: number): Promise<StockLevel> {
    const db = getDb();
    ...
    const reservedResult = await db
      .select({
        reserved: sql<number>`COALESCE(SUM(${inventory_reservations.quantity}), 0)`,
      })
      .from(inventory_reservations)
      .where(
        and(
          eq(inventory_reservations.product_variant_id, variantId),
          isNull(inventory_reservations.released_at),
        ),
      );
    ...
    for (const item of items) {
      ...
      if (item.quantity > stockLevel.available_stock) {
        throw new InventoryError(
          ...
          `Only ${stockLevel.available_stock} units available for variant ${stockLevel.sku}`,
          { available: stockLevel.available_stock, requested: item.quantity },
        );
      }
  ...
  async adjustStock(
    ...
    const newStock = Math.max(
      0,
      (variant[0].stock_quantity || 0) + quantityChange,
    );
    await db
   ...
   */
  async getOrderReservations(orderId: number): Promise<Reservation[]> {
    const db = getDb();
  ...
  async getMovementHistory(
    ...
    const results = await db
      ...
      .where(
        and(
          eq(inventory_movements.product_variant_id, variantId),
          eq(inventory_movements.store_id, storeId),
        ),
      )
  ...
  async checkLowStock(
    ...
    const lowStockVariants: StockLevel[] = [];
