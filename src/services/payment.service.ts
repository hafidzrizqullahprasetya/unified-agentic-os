...
import {
  ...
  getPaymentStatus,
  handleWebhook as processMidtransWebhook,
} from "../lib/midtrans";
...
export class PaymentService {
  ...
  async createPayment(input: CreatePaymentInput): Promise<PaymentResponse> {
    ...
    const { token, redirect_url } = await createPaymentTransaction({
      ...
      customer: {
        ...
        last_name:
          input.customerName.split(" ").slice(1).join(" ") || undefined,
   ...
   */
  async getPaymentByReferenceId(referenceId: string) {
    const db = getDb();
    const result = await db
      .select()
      .from(payments)
   ...
   */
  async handleMidtransWebhook(webhookBody: any) {
    try {
      // Process webhook and get status
      const { orderId, status, transactionStatus, statusCode } =
        await processMidtransWebhook(webhookBody);
